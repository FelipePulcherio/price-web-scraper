...
exports.scraperMain = scraperMain;
const puppeteer_core_1 = __importDefault(require("puppeteer-core"));
const config_1 = __importDefault(require("@/config"));
async function scraperMain({ storeSet, retryCount = 3, }) {
    let events = [];
            ...
            while (!success && retries < retryCount) {
                ...
                try {
                    ...
                    browser = await puppeteer_core_1.default.connect({
                        browserWSEndpoint: config_1.default.brightData,
                    });
...
async function bestbuyScraper({ page, url }) {
    //Navigate to url
    ...
    const priceLocation = await page.$('span.style-module_screenReaderOnly__4QmbS.style-module_large__g5jIz');
    // Check if element it exists
    if (priceLocation) {
        // Get the value from element
        priceContent = await page.evaluate((element) => element.textContent, priceLocation);
        if (priceContent) {
            ...
            throw new Error('Main_scraper (BB): Could not evaluate "textContent" of HTML Element.');
        }
    }
    else {
...
async function walmartScraper({ page, url }) {
    //Navigate to url
    ...
    if (priceLocation) {
        ...
        if (priceContent) {
            ...
            throw new Error('Main_scraper (WM): Could not evaluate "textContent" of HTML Element.');
        }
    }
    else {
...
async function amazonScraper({ page, url }) {
    //Navigate to url
    ...
    const priceLocation = await page.$('div.a-spacing-top-mini>span.a-price>span.a-offscreen');
    ...
    if (priceLocation) {
        ...
        if (priceContent) {
            ...
            throw new Error('Main_scraper (AM): Could not evaluate "textContent" of HTML Element.');
        }
    }
    else {
...
async function canadacomputersScraper({ page, url }) {
    //Navigate to url
    ...
    const priceLocation = await page.$('div.current-price span.current-price-value');
    // Check if element it exists
    if (priceLocation) {
        // Get the value from div
        priceContent = await page.evaluate((element) => element.textContent.trim(), priceLocation);
        if (priceContent) {
            // Remove dollar symbol
            price = parseFloat(priceContent.replace(/[$,]/g, ''));
            ...
            throw new Error('Main_scraper (CC): Could not evaluate "textContent" of HTML Element.');
        }
    }
    else {
...
async function neweggScraper({ page, url }) {
    //Navigate to url
...
//# sourceMappingURL=scraperMain.js.map